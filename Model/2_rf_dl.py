# -*- coding: utf-8 -*-
# 2_MLP.py —— Train RF + compute/save Accuracy, Precision, Recall, F1 + plot ROC/AUC
import os
...
def main():
    ...
    if not Path(PROCESSED_CSV).exists():
        raise SystemExit(f"{PROCESSED_CSV} not found. Please run the preprocessing script first.")
    ...
    # 6) Core metrics (explicit computation)
    ...
    # Save main metrics (compact table)
    ...
    # Save full classification report (per-class precision/recall/F1)
    ...
    # === 5.5 Save test-set predictions (for global ROC plotting) ===
    ...
    print(f"[SAVE] RF test predictions saved to: {save_path}")

    # 8) Top-K protein set (based on test-set probabilities)

...
PROCESSED_CSV = os.path.join(RESULT_DIR, "2_rf_dl_processed_pairs.csv")
...
def map_protein_to_symbol(ensp_list):
    ...
    if mapping.exists():
        ...
        print(f"[MAP]  id_mapping.csv  {len(syms)} 个 symbol")
        return syms
    # 在线兜底
    try:
...
def main():
    ...
    if not symbols:
        print("[GO]  gene symbols。")
        return

    # gseapy enrichr（Human）
    import gseapy as gp
    outdir = os.path.join(RESULT_DIR, "go_enrichment")
    enr = gp.enrichr(
        gene_list=symbols,
    ...
    res_path = os.path.join(outdir, "go_results.csv")
    res.to_csv(res_path, index=False)
    print(f"[SAVE] GO  - {res_path}")
    ...
    plt.title("Top 30 GO Biological Processes (Human)")
    plt.tight_layout()
    png = os.path.join(outdir, "top_go.png")
